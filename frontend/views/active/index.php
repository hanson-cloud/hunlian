...
<!-- 返回顶部调用 end-->
<!--[if lt IE 9]>
<script src="active/js/modernizr.js"></script>
<![endif]-->
    <style>
      ...
      <?php foreach ($sel as $key => $val){ ?>
          ...
          <li>
              <div class="arrow_box">
                  ...
                  <div class="ci"></div>
                  <!--圆形-->
                  <h2 class="title"><a href="?r=active/content&id=<?=$val['id']?>" target="_blank"><?=$val['appointment_title'].$val['id'];?></a></h2>
                  <ul class="textinfo">
                      <a href="/"><img src="<?=$val['appointment_image'];?>"></a>
                      <p><?=$val['appointment_content'];?></p>
                  </ul>
                  <ul class="details">
                      <li class="likes"><?=$val['man_num']?></li>
                      <li class="comments j_likes" opt="<?=$val['id']?>"><a href="javascript:void(0);"><?php if(in_array($val['id'],$user_ap)){ echo "取消加入";}else{echo "点击加入";}?></a></li>
                      <li class="icon-time"><a href="#"><?=$val['appointment_time'];?></a></li>
                  </ul>
              </div>
              <!--arrow_box end-->
          </li>
        ...
        $.ajax({
            ...
            success:function(msg){
                $.each(msg,function(k,v){
                    ...
                    str += '<h2 class="title"><a href="?r=active/content&id='+v['id']+'" target="_blank">'+v['appointment_title']+v['id']+'</a></h2>';
                    str += '<ul class="textinfo">';
                    str += '<a href="/"><img src="'+v['appointment_image']+'"></a>';
                    str += '<p>'+v['appointment_content']+'</p></ul>';
                    str += '<ul class="details">';
                    str += '<li class="likes">'+v['man_num']+'</li>';
                    str += '<li class="comments j_likes" opt="'+v['id']+'"><a href="javascript:void(0);">'+v['join']+'</a></li>';
    ...
    $(document).delegate('.j_likes','click',function(){
        ...
        $.ajax({
            ...
            success:function(msg){
                ...
                {
                    alert("一个小时之内都点它三次了，看看别的活动，"+msg+"再来吧");
                }
            }
        });
    });
